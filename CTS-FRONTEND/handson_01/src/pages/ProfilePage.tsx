  ...
  return (
    <section className="page-stack">
      ...
      <div className="profile-layout">
        <form className="section-card profile-form" onSubmit={(event) => event.preventDefault()}>
          <label className="field" htmlFor="student-name">
            Name
            <input id="student-name" value={profile.name} onChange={(event) => setProfile({ ...profile, name: event.target.value })} />
          </label>

          <label className="field" htmlFor="student-email">
            Email
            <input id="student-email" type="email" value={profile.email} onChange={(event) => setProfile({ ...profile, email: event.target.value })} />
          </label>

          <label className="field" htmlFor="student-semester">
            Semester
            <input id="student-semester" value={profile.semester} onChange={(event) => setProfile({ ...profile, semester: event.target.value })} />
          </label>

          <div className="hero-actions">
            <button type="submit">Save profile</button>
            <button type="button" className="secondary" onClick={() => dispatch(resetEnrollments())}>Clear enrollments</button>
          </div>
        </form>
            ...
            {enrolledCourses.length === 0 ? (
              ...
              enrolledCourses.map((course) => (
                <article key={course.id} className="enrollment-row">
                  ...
                  <button type="button" className="secondary" onClick={() => dispatch(unenroll(course.id))}>Remove</button>
                </article>
              ))
            )}
          </div>
